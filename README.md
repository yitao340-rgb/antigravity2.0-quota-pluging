...
4. **数据存储路径**：
   - 调试端口路径：`/Users/yitao/Library/Application Support/Antigravity/DevToolsActivePort`
   - 本地 SQLite 积分数据源：`/Users/yitao/Library/Application Support/Antigravity/User/globalStorage/state.vscdb`（解析 `antigravityUnifiedStateSync.modelCredits` Protobuf 结构）
...
当前用户是 yitao。请按照以下步骤执行：

1. 确认已创建额度插件专用目录：
   /Users/yitao/.gemini/antigravity/scratch/antigravity2.0-quota-pluging
...
3. 自动配置 launchd 守护服务：
   - 将 com.yitao.antigravity.quota.plist 复制到本地 LaunchAgents 目录：
     cp /Users/yitao/.gemini/antigravity/scratch/antigravity2.0-quota-pluging/com.yitao.antigravity.quota.plist ~/Library/LaunchAgents/com.yitao.antigravity.quota.plist
...
4. 自动触发首次调试与注入测试：
   - 执行热重载脚本，检查在当前已打开的 Antigravity 窗口中是否成功挂载额度胶囊：
     python3 /Users/yitao/.gemini/antigravity/scratch/antigravity2.0-quota-pluging/reload_quota.py
...
```bash
# 查看注入器标准输出
tail -f /Users/yitao/.gemini/antigravity/scratch/antigravity2.0-quota-pluging/launchd_output.log

# 查看运行报错信息
tail -f /Users/yitao/.gemini/antigravity/scratch/antigravity2.0-quota-pluging/launchd_error.log
```
